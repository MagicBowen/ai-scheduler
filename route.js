...
router.use('/', async function(req, res, next) {
    ...
    const access_token = _.get(body,"session.user.access_token")
    ...
    if(noResponse){
        ...
        if(requestType===0){
            ...
            if(directWakeup && query)
            {
                ...
                response = await chatbot.replyToText(userId, query, userContext);
            } else {

                console.log("receive open-app event")
                response = await chatbot.replyToEvent(userId, 'open-app',userContext);
            }
            message=messageBuilder.buildResponseSimple(response, false);
        } else if(requestType===2){
            console.log("receive close-app event")
            var response = await chatbot.replyToEvent(userId, 'close-app',userContext);
            message=messageBuilder.buildResponseSimple(response, true);
        } else{
            console.log("receive other request " + requestType + "query is: ==>" + query)
            var response = await chatbot.replyToText(userId, query, userContext);
